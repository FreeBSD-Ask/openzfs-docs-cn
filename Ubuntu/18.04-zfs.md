...
如果你遇到与本 HOWTO 相关的 bug 或功能请求，请 [创建新 issue 并提及 @rlaager](https://github.com/openzfs/openzfs-docs/issues/new?body=@rlaager,%20I%20have%20the%20following%20issue%20with%20the%20Ubuntu%2018.04%20Root%20on%20ZFS%20HOWTO:)。

### 贡献指南

1. Fork 并克隆仓库：[https://github.com/openzfs/openzfs-docs](https://github.com/openzfs/openzfs-docs)
2. 安装工具：
...
本指南支持两种加密选项：未加密和 LUKS（全盘加密）。无论哪种选项，ZFS 的所有功能均可用。Ubuntu 18.04 不支持 ZFS 原生加密。

未加密选项当然不会加密任何内容，因此性能最佳。

LUKS 会加密几乎所有数据。唯一未加密的数据是引导程序、内核和 initrd。系统必须在控制台输入密码才能启动。性能良好，但 LUKS 位于 ZFS 之下，如果使用多盘（镜像或 raidz 拓扑），数据需要在每个磁盘上加密一次。


## 第 1 步：准备安装环境

1.1 启动 Ubuntu Live CD。选择“尝试 Ubuntu”。根据需要将系统连接到互联网（例如，连接 WiFi 网络）。打开终端（按 Ctrl-Alt-T）。
...
**提示：** 可以使用 `ip addr show scope global | grep inet` 查看你的 IP 地址，然后从主机通过 `ssh ubuntu@IP` 连接。
...
使用 ZFS 时务必使用 `/dev/disk/by-id/*` 的长别名。直接使用 `/dev/sd*` 设备节点可能导致偶发的导入失败，尤其是在系统中存在多个存储池时。
...
* 选择 boot pool 大小时，请考虑实际使用。内核和 initrd 可能占用约 100M。如果安装多个内核并使用快照，尤其是在需要重新生成 initramfs（约 85M/个）时，boot pool 空间可能不足。请根据需求合理规划。
...
通常无需自定义 boot pool 的选项。

GRUB 并不支持所有 zpool 特性。请参见 [grub-core/fs/zfs/zfs.c](http://git.savannah.gnu.org/cgit/grub.git/tree/grub-core/fs/zfs/zfs.c#n276) 中的 `spa_feature_names`。此步骤为 `/boot` 创建单独的 boot pool，仅启用 GRUB 支持的特性，从而允许 root pool 使用任意特性。注意 GRUB 会以只读方式打开池，因此所有只读兼容特性均被“支持”。

**提示：**

* 如果创建镜像（mirror）或 RAID-Z（raidz）拓扑，可使用如下命令创建 boot pool：
...
* 池名称可自定义，但如果修改，必须在整个配置中保持一致。本 HOWTO 中使用 `bpool` 作为约定名称。
...
* 设置 `normalization=formD` 可消除 UTF-8 文件名的一些边缘问题，同时隐含 `utf8only=on`，意味着只允许 UTF-8 文件名。如果希望支持非 UTF-8 文件名，请不要使用该选项。[关于强制 UTF-8 文件名的问题讨论](http://utcc.utoronto.ca/~cks/space/blog/linux/ForcedUTF8Filenames)。
...
* 如果要创建镜像（mirror）或 RAIDZ 拓扑，请使用类似命令创建 root pool：
...
* ZFS pool 的名称可以任意选择。如果更改了名称，后续使用中必须保持一致。在支持自动安装到 ZFS 的系统上，root pool 默认命名为 `rpool`。
...
3.2 为 root 和 boot 文件系统创建 dataset：
...
通常情况下，ZFS 并不需要手动使用 `mount` 或 `zfs mount` 命令挂载文件系统。由于 `canmount=noauto`，这里必须手动挂载一次。
...
下面的 dataset 为可选，根据个人喜好或软件选择决定是否创建。
...
如果系统使用 Docker（管理自身 dataset 与快照）：
...
如果希望为 `/tmp` 创建独立 dataset（后续建议使用 tmpfs）：
...
该 dataset 布局的主要目的是将操作系统与用户数据分离，使得 root 文件系统可回滚而不影响用户数据。`com.sun:auto-snapshot` 设置可让部分 ZFS 快照工具排除临时数据。

如果不进行额外操作，`/tmp` 会作为 root 文件系统的一部分存储。也可以像上面示例那样创建独立 dataset，将 `/tmp` 数据排除在 root 快照之外，同时可对 `rpool/tmp` 设置空间配额限制。否则，可在后续使用 tmpfs（RAM 文件系统）。
...
`debootstrap` 命令会留下一个未配置的新系统。另一种方法是将一个已配置好的系统完整复制到新的 ZFS root 中。
...
4.4 将 LiveCD 环境的虚拟文件系统绑定到新系统并进入 chroot：
...
即便你选择非英语系统语言，也要确保 `en_US.UTF-8` 可用：
...
**提示：** 对于 HWE 内核，安装 `linux-image-generic-hwe-18.04` 替代 `linux-image-generic`。
...
**提示：** 如果创建镜像或 raidz 拓扑，需要为每个磁盘重复 `/etc/crypttab` 条目，如 `luks2` 等。
...
确保 `bpool` 始终被导入，无论 `/etc/zfs/zpool.cache` 是否存在、是否在 cachefile 中，或 `zfs-import-scan.service` 是否启用。
...
**注意**：在内存压力极高的系统上，使用 zvol 作为 swap 可能导致系统锁死，无论 swap 空间剩余多少。此问题正在调查中：[https://github.com/zfsonlinux/zfs/issues/7734](https://github.com/zfsonlinux/zfs/issues/7734)
...
压缩算法设置为 `zle`，因为它是最节省资源的算法。由于本指南推荐使用 `ashift=12`（磁盘上 4 KiB 块），常见的 4 KiB 页面大小意味着没有压缩算法能减少 I/O。例外情况是全零页面会被 ZFS 丢弃，但必须启用某种形式的压缩才能实现此行为。

7.2 配置 swap 设备：

**注意**：在配置文件中总是使用长 `/dev/zvol` 别名。切勿使用短 `/dev/zdX` 设备名。
...
`RESUME=none` 用于禁用休眠恢复。因为 zvol 在恢复脚本运行时尚未导入，所以无法正常工作。如果不禁用，启动过程会等待 swap zvol 出现约 30 秒。
...
**提示**：如果安装完整 GUI 环境，你可能需要使用 NetworkManager 管理网络：
...
本教程的大多数问题报告都涉及 `mpt2sas` 硬件，这类硬件会执行较慢的异步驱动器初始化，例如某些 IBM M1015 卡或已经刷回参考 LSI 固件的 OEM 品牌卡。
...
大多数 LSI 卡与 ZoL 完全兼容。如果你的卡存在此问题，可尝试在 `/etc/default/zfs` 中设置：
...
如果在内核日志中出现类似 `RIP: 0010:[<ffffffff8101b316>]  [<ffffffff8101b316>] native_read_tsc+0x6/0x20` 的信息，请升级或降级 Areca 驱动。ZoL 在出现此错误信息的系统上不稳定。
