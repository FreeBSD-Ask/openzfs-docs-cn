...
* 有关新的安装教程，请参阅 [Ubuntu 20.04 Root on ZFS](https://openzfs.github.io/openzfs-docs/Getting%20Started/Ubuntu/Ubuntu%2020.04%20Root%20on%20ZFS.html)。本指南已不再频繁更新，仅保留以供参考给已经按照此指南安装的用户。
...
内存少于 2 GiB 的计算机运行 ZFS，存在严重的性能瓶颈。建议 4 GiB 内存以在基础工作负载下获得正常性能。如果你想使用去重（deduplication），你将需要 [大量内存](http://wiki.freebsd.org/ZFSTuningGuide#Deduplication)。启用去重是一项不可轻易撤销的永久性更改。
