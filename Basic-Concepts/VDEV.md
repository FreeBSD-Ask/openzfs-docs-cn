...
vdev（virtual device，虚拟设备）是 ZFS 存储池的基本构建单元。它表示物理存储设备的逻辑组合，例如硬盘、SSD 或分区。

## 什么是叶子 vdev？

叶子 vdev 是最基本类型的 vdev，直接对应于物理存储设备。它是 ZFS 存储层级中的终端节点。

## 什么是顶级 vdev？

顶级 vdev 是根 vdev 的直接子节点。它们可以是单个设备，也可以是聚合多个叶子 vdev 的逻辑组（如镜像或 RAIDZ 组）。ZFS 会在池中的所有顶级 vdev 上动态条带化数据。

## 什么是根 vdev？

根 vdev 位于池层级的顶部。它将所有顶级 vdev 聚合为一个逻辑存储单元（即存储池）。

## 不同类型的 vdev

OpenZFS 支持多种类型的 vdev。顶级 vdev 承载数据并提供冗余：

* **条带磁盘（Striped Disk(s)）**：由一个或多个物理设备组成的条带化 vdev（类似 RAID 0）。不提供冗余，若磁盘故障会导致数据丢失。
* **镜像（Mirror）**：将相同数据存储在两块或多块磁盘上的 vdev，以提供冗余。
...
下面是一个示例布局，来自 [zpool-status(8)](https://openzfs.github.io/openzfs-docs/man/master/8/zpool-status.8.html) 输出，显示一个拥有两个 RAIDZ1 顶级 vdev 和 10 个叶子 vdev 的存储池：
...
ZFS 设计为能够优雅地处理 vdev 故障。如果一个 vdev 发生故障，只要存储池的冗余级别允许（例如在镜像、RAIDZ 或 dRAID 配置中），ZFS 可以继续使用池中剩余的 vdev 运行。当池中仍有足够冗余时，ZFS 会将故障的 vdev 标记为“faulted”，并从剩余的 vdev 中恢复数据。管理员可以使用 [zpool-replace(8)](https://openzfs.github.io/openzfs-docs/man/master/8/zpool-replace.8.html) 将故障 vdev 替换为新 vdev，ZFS 会自动将数据重建（resilver）到新 vdev 上，使存储池恢复健康状态。

## 如何管理 ZFS 中的 vdev？

vdev 可通过 [zpool(8)](https://openzfs.github.io/openzfs-docs/man/master/8/zpool.8.html) 命令行工具进行管理。常用操作包括：

* **创建存储池**：`zpool create` 允许指定 vdev 布局。参见 [zpool-create(8)](https://openzfs.github.io/openzfs-docs/man/master/8/zpool-create.8.html)。
...
* **更换磁盘**：`zpool replace` 用新磁盘替换故障或容量较小的磁盘。参见 [zpool-replace(8)](https://openzfs.github.io/openzfs-docs/man/master/8/zpool-replace.8.html)。
* **监控状态**：`zpool status` 显示所有 vdev 的健康状况和布局。参见 [zpool-status(8)](https://openzfs.github.io/openzfs-docs/man/master/8/zpool-status.8.html)。
* **监控性能**：`zpool iostat` 显示存储池及各 vdev 的 I/O 统计信息。参见 [zpool-iostat(8)](https://openzfs.github.io/openzfs-docs/man/master/8/zpool-iostat.8.html)。
