...
本指南支持两种加密方案：未加密和 LUKS（全盘加密）。ZFS 原生加密尚未发布。无论选择哪种方式，所有 ZFS 功能均可完全使用。
...
1.1 启动 Debian GNU/Linux Live CD。如果出现提示，请使用用户名 `user` 和密码 `live` 登录。根据需要将系统连接到互联网（例如加入你的 WiFi 网络）。
...
在 ZFS 中始终使用长的 `/dev/disk/by-id/*` 别名。直接使用 `/dev/sd*` 设备节点可能导致间歇性导入失败，尤其是在系统中存在多个存储 pool 的情况下。

**提示：**

* 使用 `ls -la /dev/disk/by-id` 可以列出别名。
...
通常无需为引导 pool 自定义任何选项。

GRUB 并不支持所有 zpool 特性。参见 [grub-core/fs/zfs/zfs.c](http://git.savannah.gnu.org/cgit/grub.git/tree/grub-core/fs/zfs/zfs.c#n276) 中的 `spa_feature_names`。此步骤为 `/boot` 创建一个单独的引导 pool，仅启用 GRUB 支持的特性，从而允许根 pool 使用任意特性。注意 GRUB 以只读方式打开 pool，因此所有兼容只读的特性都被 GRUB 支持。
...
* pool 名称可自定义。如果更改，需在整个操作中保持一致。`bpool` 命名约定源自本 HOWTO。
...
* 设置 `normalization=formD` 可消除一些与 UTF-8 文件名规范化相关的边缘情况，同时隐含 `utf8only=on`，意味着只允许 UTF-8 文件名。如果希望支持非 UTF-8 文件名，请勿使用此选项。关于强制 UTF-8 文件名可能带来的问题，请参见 [The problems with enforced UTF-8 only filenames](http://utcc.utoronto.ca/~cks/space/blog/linux/ForcedUTF8Filenames)。
* 设置 `relatime=on` 是经典 POSIX `atime` 行为（性能影响较大）和 `atime=off`（完全禁用 atime 更新以获得最佳性能）之间的折中。自 Linux 2.6.30 起，其他文件系统默认启用 `relatime`。详情请参见 [RedHat 文档](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/6/html/power_management_guide/relatime)。
* 设置 `xattr=sa` 可显著提升扩展属性性能 [performance of extended attributes](https://github.com/zfsonlinux/zfs/commit/82a37189aac955c81a59a5ecc3400475adb56355)。在 ZFS 内部，扩展属性用于实现 POSIX ACL，也可被用户空间应用使用。[部分桌面 GUI 应用使用扩展属性](https://en.wikipedia.org/wiki/Extended_file_attributes#Linux)。[Samba 可利用扩展属性存储 Windows ACL 和 DOS 属性；Samba Active Directory 域控制器也需要它们](https://wiki.samba.org/index.php/Setting_up_a_Share_Using_Windows_ACLs)。请注意，`xattr=sa` 是 Linux 特有的。如果将 `xattr=sa` pool 移动到除 ZFS-on-Linux 之外的 OpenZFS 实现，扩展属性将不可读（数据本身仍可读）。若扩展属性的可移植性对你很重要，请省略上述 `-O xattr=sa`。即便不为整个 pool 使用 `xattr=sa`，在 `/var/log` 使用通常没有问题。
...
* pool 名称可自定义。如果更改，需在整个操作中保持一致。在可自动安装到 ZFS 的系统上，根 pool 默认命名为 `rpool`。

## 第三步：系统安装

3.1 创建作为容器的文件系统 dataset：
...
3.2 为根文件系统和引导文件系统创建文件系统 dataset：
...
如果系统将使用 Docker（管理自己的 datasets 和快照）：
...
建议之后使用 tmpfs，但如果希望为 `/tmp` 创建单独的 dataset：
...
此 dataset 布局的主要目标是将操作系统与用户数据分离。这样可以在回滚根文件系统时，不影响用户数据，如 `/var/log` 中的日志。这在将来集成 `beadm` 或类似工具时尤为重要。`com.sun.auto-snapshot` 设置被一些 ZFS 快照工具用来排除临时数据。

如果不做额外操作，`/tmp` 将作为根文件系统的一部分存储。或者，你可以像上文示例那样为 `/tmp` 创建单独的 dataset，这样 `/tmp` 数据不会包含在根文件系统的快照中。还可以对 `rpool/tmp` 设置配额，以限制最大使用空间。否则，可以后续使用 tmpfs（RAM 文件系统）。
...
`debootstrap` 命令会将新系统保持在未配置状态。替代方法是将一个可用系统的全部内容复制到新的 ZFS 根中。
...
即使你偏好非英语系统语言，也需确保 `en_US.UTF-8` 可用。
...
**提示：** 如果创建镜像或 raidz 拓扑，请为 `luks2` 等磁盘重复 `/etc/crypttab` 条目。
...
**注意：** 如果创建镜像或 raidz 拓扑，此步骤仅在第一块磁盘上安装 GRUB，其他磁盘稍后处理。
...
这可确保 `bpool` 始终被导入，无论 `/etc/zfs/zpool.cache` 是否存在、是否在缓存文件中，或 `zfs-import-scan.service` 是否启用。
...
如果之前创建了 `/tmp` dataset，则跳过此步骤，因为两者互斥。否则，可以通过启用 `tmp.mount` 单元，将 `/tmp` 放在 tmpfs（RAM 文件系统）上。
...
`popularity-contest` 软件包会报告系统上安装的软件列表。显示 ZFS 的受欢迎程度有助于获得发行版长期关注。
...
**注意：** 使用 LUKS 时，可能会显示 “WARNING could not determine root device from /etc/fstab”。这是因为 [cryptsetup 不支持 ZFS](https://bugs.launchpad.net/ubuntu/+source/cryptsetup/+bug/1612906)。

5.3 解决 GRUB 对 zpool-features 支持缺失的问题：
...
在收到完全相同的结果信息前，请不要重启计算机。注意，此操作是将 GRUB 安装到整个磁盘，而非分区。
...
在 initramfs 中挂载 `/boot` 之前，也需要挂载它，因为它被标记为 `canmount=noauto`。对于 UEFI，还需确保在其子文件系统 `/boot/efi` 挂载前挂载它。

`rpool` 已保证被 initramfs 导入，因此无需为这些文件系统添加 `x-systemd.requires=zfs-import.target`。
...
**注意**：在内存压力极高的系统上，使用 zvol 作为交换设备可能导致系统锁死，无论交换空间还有多少可用。目前该问题正在调查中：[https://github.com/zfsonlinux/zfs/issues/7734](https://github.com/zfsonlinux/zfs/issues/7734)
...
压缩算法设置为 `zle`，因为这是最便宜的算法。由于本指南推荐使用 `ashift=12`（磁盘上 4 KiB 块），对于常见的 4 KiB 页大小，没有压缩算法能降低 I/O。例外情况是全零页，ZFS 会丢弃它们；但必须启用某种压缩才能实现此行为。

7.2 配置交换设备：

**注意**：在配置文件中始终使用长 `/dev/zvol` 别名，绝不要使用短 `/dev/zdX` 设备名。
...
`RESUME=none` 用于禁用从休眠恢复。因为在恢复脚本运行时 zvol 尚未导入（pool 未导入），所以如果不禁用，启动过程会等待交换 zvol 出现而挂起 30 秒。
...
**注意**：默认会勾选“Debian 桌面环境”和“打印服务器”。如果希望安装服务器版本，请取消勾选这些选项。
...
本教程的大多数问题报告涉及 `mpt2sas` 硬件，这类硬件会进行缓慢的异步磁盘初始化，例如一些 IBM M1015 或刷成参考 LSI 固件的 OEM 卡。

基本问题是，这些控制器上的磁盘在 Linux 内核启动常规系统之前不可见，而 ZoL 不支持热插拔 pool 成员。详见 [https://github.com/zfsonlinux/zfs/issues/330](https://github.com/zfsonlinux/zfs/issues/330)。

大多数 LSI 卡与 ZoL 完全兼容。如果你的卡存在此问题，尝试在 `/etc/default/zfs` 中设置 `ZFS_INITRD_PRE_MOUNTROOT_SLEEP=X`。系统将在导入 pool 前等待 X 秒以确保所有磁盘出现。

### Areca

需要 `arcsas` blob 驱动的系统，应将其添加到 `/etc/initramfs-tools/modules` 文件，并运行 `update-initramfs -u -k all`。
