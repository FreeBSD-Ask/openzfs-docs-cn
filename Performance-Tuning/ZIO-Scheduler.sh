...
ZFS 向叶子 vdevs （ 通常为设备 ）发出 I / O 操作以满足并完成 I / O。ZIO 调度器决定这些操作何时以及以何种顺序被发出。操作被划分为九个 I / O 类，优先级按如下顺序排列：
...
对于非交互式 I / O （ scrub、resilver、removal、initializing 和 rebuild ），并发活动 I / O 数量限制为 _min_active，除非 vdev 处于 “idle” 状态。当没有交互式 I / O （ sync 或 async ）处于活动状态，并且自上次交互式 I / O 以来已完成 zfs_vdev_nia_delay 个 I / O 时，vdev 被视为 “idle”，此时并发活动的非交互式 I / O 数量将增加到 _max_active。
...
I / O 队列统计信息包含大多数 I / O 类，可通过 zpool iostat -q 命令查看。

对于许多物理设备而言，吞吐量会随着并发操作数量增加而提升，但延迟通常会增加。此外，物理设备通常存在一个上限，超过该上限后增加并发操作不会提升吞吐量，甚至可能导致磁盘性能下降。

ZIO 调度器在选择下一个要发出的操作时，首先查找其最小值尚未满足的 I / O 类。一旦全部满足且未达到聚合最大值，调度器会查找其最大值尚未满足的类。I / O 类的遍历顺序按上述优先级进行。如果已达到聚合最大并发操作数，或对于尚未达到最大值的 I / O 类没有排队操作，则不会发出更多操作。每当有 I / O 被加入队列或某个操作完成时，I / O 调度器都会查找新的可发出操作。

一般而言，较小的 max_active 值会降低同步操作的延迟。较大的 max_active 值可能会提高整体吞吐量，这取决于底层存储和 I / O 负载组合。
...
除 async write 类外，所有 I / O 类都有固定的未完成操作最大数量。异步写入表示在事务组同步阶段提交到稳定存储的数据。事务组会周期性进入同步状态，因此排队的 async write 数量会迅速突增，然后降为零。zfs_txg_timeout 可调参数（ 默认值 = 5 秒 ）设置 txg 同步的目标间隔。因此，每 5 秒一次的 async write 突发是正常的 ZFS I / O 模式。
...
代码参考： [vdev_queue.c](https://github.com/openzfs/zfs/blob/master/module/zfs/vdev_queue.c#L42)
