...
OpenZFS 是卓越的存储平台，拥有传统文件系统、卷管理器等功能，且在所有发行版中都能提供一致的可靠性、功能性和性能。可以在 [OpenZFS 维基百科条目](https://en.wikipedia.org/wiki/OpenZFS) 中找到更多有关 OpenZFS 的信息。
...
在需要最强数据完整性保障的企业环境中，强烈建议为 OpenZFS 搭配 ECC 内存。在没有 ECC 内存时，由宇宙射线或故障内存引起的罕见随机位翻转可能无法被检测到。只要发生这种情况，OpenZFS（及任何其他文件系统）都会把受损的数据写入磁盘，且无法自动检测到这种损坏。
...
某个 OpenZFS 版本的 [说明](https://github.com/openzfs/zfs/releases) 中会包含受支持内核的范围。根据需要，点发布版本会被打上标签，以支持来自 [kernel.org](https://www.kernel.org/) 的 *stable*（稳定版）内核。由于在企业级 Linux 发行版中的重要地位，最早支持的内核版本是 2.6.32。

## 32 位 与 64 位 系统

**极其建议** 使用 64 位内核。可以在 32 位 系统上构建 OpenZFS，但你可能会遇到稳定性问题。
