...
* 请勿将这些说明用于双系统启动。
* 备份你的数据。任何现有数据都将丢失。
* 这不是 openSUSE 官方 HOWTO 页面。如果将来 openSUSE 增加对 Root on ZFS 的支持，本文档将会更新。此外，[openSUSE 的默认系统安装器 Yast2 不支持 zfs](https://forums.opensuse.org/showthread.php/510071-HOWTO-Install-ZFSonLinux-on-OpenSuse)。本页面中在不使用 Yast2 的情况下通过 zypper 设置系统的方法，基于社区中人员经验所撰写的 openSUSE 安装方法。有关更多信息，请查看外部链接。
...
内存少于 2 GiB 的计算机运行 ZFS 会很慢。对于基础工作负载，建议至少使用 4 GiB 内存以获得正常性能。如果你希望使用去重功能，将需要[大量的 RAM](http://wiki.freebsd.org/ZFSTuningGuide#Deduplication)。启用去重是一项永久性更改，无法轻易还原。

### 支持

如果你需要帮助，可以通过 [邮件列表](https://openzfs.github.io/openzfs-docs/Project%20and%20Community/Mailing%20Lists.html#mailing-lists) 或在 [Libera Chat](https://libera.chat/) 上的 IRC 频道 [#zfsonlinux](ircs://irc.libera.chat/#zfsonlinux) 联系社区。如果你有与本 HOWTO 相关的 Bug 报告或功能请求，请提交一个新的 issue，并提及 [@Zaryob](https://github.com/Zaryob)。
...
4. 测试：
   ...
   ```
5. 在一个分支上执行 `git commit --signoff`，然后 `git push`，并创建一个 pull request。

### 加密

本指南支持三种不同的加密选项：未加密、ZFS 原生加密、以及 LUKS。无论选择哪一种，所有 ZFS 特性都可以完全使用。

未加密当然不会加密任何内容。在没有进行任何加密的情况下，该选项自然具有最佳性能。
...
LUKS 会加密几乎所有内容。唯一未加密的数据是 bootloader、kernel 和 initrd。系统在未在控制台输入口令的情况下无法启动。性能良好，但 LUKS 位于 ZFS 之下，因此如果使用了多个磁盘（mirror 或 raidz 拓扑），数据需要在每个磁盘上各加密一次。
...
1. 启动 openSUSE Live CD。如有提示，请使用用户名 `linux` 登录且不设置密码。根据需要将系统连接到互联网（例如加入你的 WiFi 网络）。打开一个终端。
...
4. 可选：在 Live CD 环境中安装并启动 OpenSSH 服务器：
   ...
   **提示：** 你可以使用 `ip addr show scope global | grep inet` 查找你的 IP 地址。然后，在你的主机上，通过 `ssh user@IP` 进行连接。不要忘记通过 `passwd` 为用户设置密码。
5. 禁用自动挂载：
...
1. 设置磁盘变量：
   ...
   在 ZFS 中始终使用长 `/dev/disk/by-id/*` 别名。直接使用 `/dev/sd*` 设备节点可能导致偶发的导入失败，尤其是在系统中存在多个存储池时。
   **提示：**
...
4. 创建启动池：
   ...
   对启动池无需自定义选项。
   GRUB 不支持所有 zpool 功能。请参见 [grub-core/fs/zfs/zfs.c](http://git.savannah.gnu.org/cgit/grub.git/tree/grub-core/fs/zfs/zfs.c#n276) 中的 `spa_feature_names`。
   此步骤为 `/boot` 创建单独的启动池，其功能仅限于 GRUB 支持的功能，从而允许根池使用任意功能。注意 GRUB 以只读方式打开池，因此所有只读兼容功能均被 GRUB “支持”。
   **提示：**
   ...
   * 池名称可自定义。若更改，新名称必须保持一致使用。`bpool` 命名约定来源于本 HOWTO。
...
* 设置 `normalization=formD` 可消除一些与 UTF-8 文件名规范化相关的边缘情况。它还隐含 `utf8only=on`，意味着只允许 UTF-8 文件名。如果你希望支持非 UTF-8 文件名，请不要使用此选项。关于强制仅使用 UTF-8 文件名可能存在的问题，请参见 [The problems with enforced UTF-8 only filenames](http://utcc.utoronto.ca/~cks/space/blog/linux/ForcedUTF8Filenames)。
...
* 设置 `relatime=on` 在经典 POSIX `atime` 行为（性能影响显著）与 `atime=off`（完全禁用 atime 更新以获得最佳性能）之间提供折中。自 Linux 2.6.30 起，`relatime` 成为其他文件系统的默认设置。更多信息请参见 [RedHat 文档](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/6/html/power_management_guide/relatime)。
...
* 池名称可自定义。如果更改，新名称必须保持一致。在可以自动安装到 ZFS 的系统中，根池默认命名为 `rpool`。
...
1. 创建用于容器的文件系统数据集：
   ...
   在 Solaris 系统中，根文件系统会被克隆，并通过 `pkg image-update` 或 `beadm` 对后缀进行递增以应对重大系统更改。类似功能已在 Ubuntu 20.04 中通过 `zsys` 工具实现，尽管其数据集布局更复杂。即使没有此类工具，rpool/ROOT 和 bpool/BOOT 容器仍可用于手动创建的克隆。不过，为简化操作，本 HOWTO 假设 `/boot` 使用单一文件系统。
...
3. 创建其他数据集：
   ...
   此数据集布局的主要目标是将操作系统与用户数据分离，从而允许回滚根文件系统而不影响用户数据。
   如果不做额外操作，`/tmp` 将作为根文件系统的一部分存储。或者，可以如上创建独立的 `/tmp` 数据集，将 `/tmp` 数据排除在根文件系统快照之外。同时，可对 `rpool/tmp` 设置配额以限制最大使用空间。否则，可在后续使用 tmpfs（RAM 文件系统）。
...
6. 安装内核：
   ...
   **注意：** 如果安装了 base pattern，需要卸载 busybox-grep 才能安装 kernel-default 包。
...
15. 可选（但推荐）：将 `/tmp` 挂载为 tmpfs
    如果之前创建了 `/tmp` dataset，则跳过此步，否则可以通过启用 `tmp.mount` 单元将 `/tmp` 放在 tmpfs（内存文件系统）上：
...
4. 刷新 initrd 文件：
   ...
   **注意：** 在某些安装中，LUKS 分区可能无法被 dracut 识别，会出现 “Failure occurred during following action: configuring encrypted DM device X VOLUME_CRYPTSETUP_FAILED” 错误。
   ...
   **注意：** 即使已将 zfs 配置添加到系统模块 `/etc/modules.d`，若 dracut 未识别，仍需强制添加：
...
**警告：** 这将破坏你的 Yast2 启动加载器配置。仅在无法通过 grub2 修复问题时才使用此方法。本步骤是为某些情况下 grub2 无法识别 rpool 池而提供的替代方案。
...
**注意**：在内存压力极高的系统上，使用 zvol 作为 swap 设备可能导致系统锁死，无论 swap 空间剩余多少。[上游已有相关 bug 报告](https://github.com/zfsonlinux/zfs/issues/7734)。
...
2. 配置 swap 设备：
   **注意**：在配置文件中始终使用完整 `/dev/zvol` 别名，切勿使用短设备名 `/dev/zdX`。
